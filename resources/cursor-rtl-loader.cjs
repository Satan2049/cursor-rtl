(function() {
    ...
    log("pid=" + process.pid, "version=1.0.0");
    ...
    function findRtlScript() {
        ...
        try {
            ...
            var dirs = entries
                .filter(function(d) {
                    if (!/cursor-rtl/i.test(d)) return false;
                    var rtlPath = path.join(extDir, d, "resources", "rtl.js");
                    return fs.existsSync(rtlPath);
                })
                .sort();
            log("cursor-rtl dirs found:", JSON.stringify(dirs));
            if (dirs.length > 0) {
                var rtlPath = path.join(extDir, dirs[dirs.length - 1], "resources", "rtl.js");
                log("rtl.js path:", rtlPath, "exists:", fs.existsSync(rtlPath));
                return rtlPath;
            }
        } catch (e) {
